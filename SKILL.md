...
- For `implement`, use `scripts/implement_runtime.py --repo <repo-root> --track <track-id> --action <start|complete_task|commit_task|checkpoint_phase>` to advance the runtime state through track start, task progression, task-commit choreography, phase-checkpoint verification, completion, doc-sync handoff, and cleanup handoff.
