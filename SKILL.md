...
- Use bundled scripts for deterministic support of setup analysis, draft generation, track lifecycle updates, review/revert preparation, and skills installation.
...
7. Use `scripts/draft_setup_docs.py --repo <repo-root>` to generate starting drafts for shared context files when guided setup needs a first-pass document.
8. Ask only for missing or preference-driven context, including track Git workflow policy and coverage target.
9. Capture and approve the workflow policy that should be written into or preserved in `conductor/workflow.md`.
...
10. Produce a structured preview of the proposed live workspace artifacts and approved workflow decisions before writing files.
11. Require explicit user confirmation.
12. If an official-style workspace already exists, preserve its shared context files and only fill missing artifacts.
13. If a legacy Codex-native workspace is detected, stop and require migration before continuing.
14. Only after confirmation, run `scripts/bootstrap_conductor.py --repo <repo-root>` to materialize or repair the agreed canonical workspace.
15. Be ready to hand off into the first track flow when appropriate.
...
6. Use `scripts/draft_new_track.py --repo <repo-root> --track-id <track-id> --title "<title>"` to produce first-pass `spec.md` and `plan.md` drafts before interactive refinement.
7. Continue by refining the generated `spec.md` and `plan.md`.
8. Keep `plan.md` compatible with upstream Conductor expectations: phases as main headings, `[ ]`, `[~]`, `[x]` task markers, and room for task SHA and phase checkpoint annotations.
9. Use `scripts/skills_catalog.py` and `scripts/install_skills.py` when the track flow recommends skill installation.
